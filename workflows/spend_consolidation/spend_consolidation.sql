...
DROP TABLE IF EXISTS tajawal_uat_bi.fact_spend_data_channelwise_inter;
CREATE TABLE IF NOT EXISTS tajawal_uat_bi.fact_spend_data_channelwise_inter as
select
`date` as dim_date_id,
ds.channel_group,
ds.channel,
...
INSERT INTO tajawal_uat_bi.fact_spend_data_channelwise_inter
select
`date` as dim_date_id,
ds.channel_group,
ds.channel,
...
(
  ...
  where dim_bookingdate_id >= '20171101' and ds.type = 'app' 
) A 
inner join 
(
  ...
  where lower(dsc.channel_group) like '%mobile%network%' and 
...
left outer join tajawal_uat_bi.dim_spend_fb_accounts fba on fb.account_id = fba.account_id
...
(
    ...
    (
        ...
        dim_group_id,
        d.pos
        from tajawal_uat_bi.fact_hotel_orders_final h
        inner join tajawal_uat_bi.dim_store d on h.dim_store_id=d.store_id
    ...
    (
        ...
        dim_group_id 
        from 
        tajawal_uat_bi.fact_hotel_orders_final
...
(
    ...
    from tajawal_uat_bi.fact_trivago_hotel_data 
...
(
    ...
    from tajawal_uat_bi.dim_spend_channels
    group by dim_channel_id
...
DROP TABLE IF EXISTS tajawal_uat_bi.fact_spend_data_channelwise_inter2;


-- Step 6 : Combine all toghether
DROP TABLE IF EXISTS tajawal_uat_bi.fact_spend_data_channelwise_inter2;
CREATE TABLE IF NOT EXISTS tajawal_uat_bi.fact_spend_data_channelwise_inter2 as
select 
dim_date_id,
...
from
tajawal_uat_bi.fact_spend_data_channelwise_inter
group by 
dim_date_id,
...
drop table if exists tajawal_uat_bi.fact_spend_data_channelwise;
alter table tajawal_uat_bi.fact_spend_data_channelwise_inter2 rename to tajawal_uat_bi.fact_spend_data_channelwise;
exit;
-- ===============
...
drop table if exists tajawal_uat_bi.report_tajawal_funnel_inter_inter;
create table if not exists tajawal_uat_bi.report_tajawal_funnel_inter_inter row format delimited fields terminated by '\t'
as
select 
`date` as dim_date, 
b.group_name,
...
create table if not exists tajawal_uat_bi.report_tajawal_funnel_inter_inter2
...
from
tajawal_uat_bi.report_tajawal_funnel_inter_inter
group by
...
drop table if exists tajawal_uat_bi.report_tajawal_funnel;
alter table tajawal_uat_bi.report_tajawal_funnel_inter_inter2 rename to tajawal_uat_bi.report_tajawal_funnel;


drop table if exists tajawal_uat_bi.fact_per_city_orders_searches;

create table tajawal_uat_bi.fact_per_city_orders_searches ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
as
