...
drop table if exists ${hiveDB}.fact_adjust_data_multi_app_final_incr;
...
Create table if not exists ${hiveDB}.fact_adjust_data_multi_app_final_incr ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' 
...
(case when lower(campaign) like '%ar\_%' or lower(campaign) like '%\_ar%' then 'ar' else case when lower(campaign) like '%en\_%' or lower(campaign) like '%\_en%' then 'en' else 'NA' end end) as language,
  ...
  from ${hiveDB}.fact_adjust_data_multi_app_incr f
...
from ${hiveDB}.fact_adjust_data_multi_app_final_incr A inner join ${hiveDB}.dim_spend_adjust_apps B on A.app_token = B.dim_app_id
inner join
(
...
Drop table if exists ${hiveDB}.fact_mobilenetwork_data_incr;
Create table if not exists ${hiveDB}.fact_mobilenetwork_data_incr row format delimited fields terminated by ',' as
...
drop table if exists  ${hiveDB}.fact_adjust_active_users_incr;
create table if not exists ${hiveDB}.fact_adjust_active_users_incr ROW FORMAT DELIMITED FIELDS TERMINATED BY ','
as
select 
apps.app_name,
...
from
${hiveDB}.fact_adjust_data_multi_app_final_incr adjust 
