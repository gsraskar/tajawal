...
drop table if exists tajawal_uat_bi.tmp_mobile_adnetwork;
create table if not exists tajawal_uat_bi.tmp_mobile_adnetwork as
...
drop table tajawal_uat_bi.tmp_mobile_adnetwork_inter;
create table tajawal_uat_bi.tmp_mobile_adnetwork_inter as select * from tajawal_uat_bi.tmp_mobile_adnetwork;
-- Include data for mobile_os = 'all'

insert into tajawal_uat_bi.tmp_mobile_adnetwork_inter
...
( 
  ...
  (
    ...
    from tajawal_uat_bi.tmp_mobile_adnetwork where not `date` like '%date%'
  ...
  (
    ...
    from tajawal_uat_bi.tmp_mobile_adnetwork where mobile_os='all' and not `date` like '%date%'
    group by adnetwork_name, `date`, metric_name, brand, type, product
  ) q2
...
drop table tajawal_uat_bi.tmp_mobile_adnetwork;
alter table tajawal_uat_bi.tmp_mobile_adnetwork_inter rename to tajawal_uat_bi.tmp_mobile_adnetwork;
create table tajawal_uat_bi.tmp_mobile_adnetwork_inter as select * from tajawal_uat_bi.tmp_mobile_adnetwork;
insert into tajawal_uat_bi.tmp_mobile_adnetwork_inter
-- union
...
left outer join
(select adnetwork_name, `date`, metric_name, brand, type, product,max(value) as value
from tajawal_uat_bi.tmp_mobile_adnetwork where mobile_os='ios' and not `date` like '%date%'
...
left outer join
(select adnetwork_name, `date`, metric_name, brand, type, product, value
from tajawal_uat_bi.tmp_mobile_adnetwork where mobile_os='android' and not `date` like '%date%'
) q2
on q1.adnetwork_name=q2.adnetwork_name and q1.`date`=q2.`date` and q1.metric_name=q2.metric_name 
and q1.brand=q2.brand and q1.type=q2.type and q1.product=q2.product
) q4
...
alter table tajawal_uat_bi.tmp_mobile_adnetwork_inter rename to tajawal_uat_bi.tmp_mobile_adnetwork;
-- exit;
create table tajawal_uat_bi.tmp_mobile_adnetwork_inter as 

select * from tajawal_uat_bi.tmp_mobile_adnetwork
-- insert into tajawal_uat_bi.tmp_mobile_adnetwork_inter
union
-- create table tajawal_uat_bi.tmp_mobile_adnetwork_inter as
...
(
  ...
  from tajawal_uat_bi.tmp_mobile_adnetwork where product='all' and not `date` like '%date%'
  group by adnetwork_name, `date`, metric_name, brand, type, mobile_os
) q2
on q1.adnetwork_name=q2.adnetwork_name and q1.`date`=q2.`date` and q1.metric_name=q2.metric_name 
...
( 
  ...
  (
    ...
    from tajawal_uat_bi.tmp_mobile_adnetwork where product='package' and not `date` like '%date%'
  ) q2
  on q1.adnetwork_name=q2.adnetwork_name and q1.`date`=q2.`date` and q1.metric_name=q2.metric_name 
  and q1.brand=q2.brand and q1.type=q2.type and q1.mobile_os=q2.mobile_os
) q4
...
( 
  ...
  (
    select adnetwork_name, `date`, metric_name, brand, type, mobile_os,value
    from tajawal_uat_bi.tmp_mobile_adnetwork where product='hotel' and not `date` like '%date%'
  ) q2
  on q1.adnetwork_name=q2.adnetwork_name and q1.`date`=q2.`date` and q1.metric_name=q2.metric_name 
  and q1.brand=q2.brand and q1.type=q2.type and q1.mobile_os=q2.mobile_os
) q4
...
( 
  ...
  (
    select adnetwork_name, `date`, metric_name, brand, type, mobile_os, value
    from tajawal_uat_bi.tmp_mobile_adnetwork where product='flight' and not `date` like '%date%'
  ) q2
  on q1.adnetwork_name=q2.adnetwork_name and q1.`date`=q2.`date` and q1.metric_name=q2.metric_name 
  and q1.brand=q2.brand and q1.type=q2.type and q1.mobile_os=q2.mobile_os
) q4
...
from tajawal_uat_bi.tmp_mobile_adnetwork
group by  adnetwork_name, `date`, metric_name, brand, type, mobile_os;

drop table tajawal_uat_bi.tmp_mobile_adnetwork;
alter table  tajawal_uat_bi.tmp_mobile_adnetwork_inter rename to  tajawal_uat_bi.tmp_mobile_adnetwork;

drop table if exists tajawal_uat_bi.tmp_mobile_adnetwork_cost;
create table if not exists tajawal_uat_bi.tmp_mobile_adnetwork_cost as
...
from 
(
    select adnetwork_name, `date`, metric_name, hotel_cost,flight_cost,package_cost,all_cost, brand,type,mobile_os from tajawal_uat_bi.tmp_mobile_adnetwork where `date` != 'date'
    union
    ...
    (select adnetwork_name, brand, type,mobile_os, count(*) from tajawal_uat_bi.tmp_mobile_adnetwork where `date` != 'date' group by adnetwork_name, brand, type,mobile_os) T
) T1 
where lower(brand) like '%almosafer%'
order by adnetwork_name, cost_date asc
) A;

insert into tajawal_uat_bi.tmp_mobile_adnetwork_cost
select A.adnetwork_name,A.mobile_os,A.cost_date as start_date,
LEAD(cost_date, 1, from_unixtime(unix_timestamp(CURRENT_TIMESTAMP(), 'yyyy-MM-dd hh:mm:ss.S'), 'yyyyMMdd')) OVER (PARTITION BY adnetwork_name,mobile_os ORDER BY cost_date) as end_date,
metric_name, hotel_cost,flight_cost,package_cost,all_cost, brand, type
from
...
-- drop table if exists tajawal_uat_bi.fact_adjust_data_multi_app_inter;
drop table if exists tajawal_uat_bi.fact_adjust_data_multi_app_final_incr;
-- alter table tajawal_uat_bi.fact_adjust_data_multi_app rename to tajawal_uat_bi.fact_adjust_data_multi_app_inter;
--Drop table if exists tajawal_uat_bi.fact_adjust_data;
Create table if not exists tajawal_uat_bi.fact_adjust_data_multi_app_final_incr ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' 
...
select
  ...
  COALESCE(COALESCE(`flight_revenue`,0.0)+COALESCE(`flight_domestic_revenue`,0.0),0.0) as flight_revenue, 
  COALESCE(COALESCE(`flight_events`,0.0)+COALESCE(`flight_domestic_events`,0.0),0.0) as flight_events, 
...
(case when lower(campaign) like '%ar\_%' or lower(campaign) like '%\_ar%' then 'ar' else case when lower(campaign) like '%en\_%' or lower(campaign) like '%\_en%' then 'en' else 'NA' end end) as language,
  ...
  from tajawal_uat_bi.fact_adjust_data_multi_app_incr f
  ...
  from tajawal_uat_bi.criteo_campaign_mapping group by lower(ltrim(rtrim(ga_adjust_campaign))) ) ccm 
  on lower(ltrim(rtrim(f.campaign))) = ccm.ga_adjust_campaign
  inner join tajawal_uat_bi.dim_spend_adjust_apps aa on f.app_token = aa.dim_app_id 
  left outer join (select lower(regexp_replace(adjust_network_name, ' ', '')) as adjust_network_name, max(dim_channel_id) as dim_channel_id from tajawal_uat_bi.dim_spend_adjust_network_channels group by lower(regexp_replace(adjust_network_name, ' ', ''))) an on lower(regexp_replace(f.network, ' ', '')) = lower(regexp_replace(an.adjust_network_name, ' ', ''))
  left outer join tajawal_uat_bi.dim_spend_channels ds on an.dim_channel_id = ds.dim_channel_id
  left outer join tajawal_uat_bi.tmp_mobile_adnetwork_cost c on lower(rtrim(ltrim(regexp_replace(f.network, ' ', '')))) = lower(rtrim(ltrim(regexp_replace(c.adnetwork_name, ' ', '')))) and ltrim(rtrim(lower(aa.mobile_os)))=ltrim(rtrim(lower(c.mobile_os))) and
...
CREATE TABLE IF NOT EXISTS tajawal_uat_bi.dim_spend_channels (
...
DROP TABLE IF EXISTS tajawal_uat_bi.dim_spend_channels_inter;
CREATE TABLE IF NOT EXISTS tajawal_uat_bi.dim_spend_channels_inter row format delimited fields terminated by ',' as
-- SELECT *
...
(
    -- select * from
    -- tajawal_uat_bi.dim_spend_channels
    -- union
    select * from
    (
        ...
        -- left outer join tajawal_uat_bi.dim_spend_channels ds 
        -- ON A.channel_group = ds.channel_group and A.channel = ds.channel
        -- where ds.channel is null
    ) T2
) T group by dim_channel_id;
DROP TABLE tajawal_uat_bi.dim_spend_channels;
ALTER TABLE tajawal_uat_bi.dim_spend_channels_inter RENAME TO tajawal_uat_bi.dim_spend_channels;

-- ==== Step 2 : Merge new network=>channel mapping into dim_adjust_network table ==== --
-- Create table dim_spend_adjust_network_channels
CREATE TABLE IF NOT EXISTS tajawal_uat_bi.dim_spend_adjust_network_channels (
    dim_network string,
    dim_channel_id string
);
-- POPULATE table with new channels.
DROP TABLE IF EXISTS tajawal_uat_bi.dim_spend_adjust_network_channels_inter;
CREATE TABLE IF NOT EXISTS tajawal_uat_bi.dim_spend_adjust_network_channels_inter row format delimited fields terminated by ',' as
SELECT *
FROM
(
--    select * from
--    (
--        select * from tajawal_uat_bi.dim_spend_adjust_network_channels
--    ) T1
--    union
    ...
    (
        ...
        from googlesheets.dim_adjust_networks da inner join tajawal_uat_bi.dim_spend_channels ds 
...
--    left outer join tajawal_uat_bi.dim_spend_adjust_network_channels dsa on T2.dim_channel_id = dsa.dim_channel_id
--    where dsa.dim_channel_id is null
) T;
DROP TABLE tajawal_uat_bi.dim_spend_adjust_network_channels;
ALTER TABLE tajawal_uat_bi.dim_spend_adjust_network_channels_inter RENAME TO tajawal_uat_bi.dim_spend_adjust_network_channels;

Drop table if exists tajawal_uat_bi.fact_mobilenetwork_data_detailed;
Create table if not exists tajawal_uat_bi.fact_mobilenetwork_data_detailed row format delimited fields terminated by ','  as
select C.channel_group, C.channel, B.group_name, A.*
from tajawal_uat_bi.fact_adjust_data_multi_app_final_incr A inner join tajawal_uat_bi.dim_spend_adjust_apps B on A.app_token = B.dim_app_id
inner join
(
    select B.channel_group, B.channel, A.dim_channel_id, A.adjust_network_name, count(*) 
    from tajawal_uat_bi.dim_spend_adjust_network_channels A inner join tajawal_uat_bi.dim_spend_channels B 
...
Drop table if exists tajawal_uat_bi.fact_mobilenetwork_data_incr;
Create table if not exists tajawal_uat_bi.fact_mobilenetwork_data_incr row format delimited fields terminated by ',' as
...
drop table if exists  tajawal_uat_bi.fact_adjust_active_users_incr;
create table if not exists tajawal_uat_bi.fact_adjust_active_users_incr ROW FORMAT DELIMITED FIELDS TERMINATED BY ','
as
select 
apps.app_name,
...
from
tajawal_uat_bi.fact_adjust_data_multi_app_final_incr adjust 
left outer join tajawal_uat_bi.dim_spend_adjust_apps apps 
on apps.dim_app_id=adjust.app_token
left outer join tajawal_uat_bi.dim_spend_channels channels
