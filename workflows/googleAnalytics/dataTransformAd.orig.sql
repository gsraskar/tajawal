...
(
  ...
  case when lower(campaign) like '%\_sa%' or lower(campaign) like '%\-sa\-%' or lower(campaign) like '%\_ks%' or lower(campaign) like '%ksa%' or  (lower(campaign) like '%sa\_%' and lower(campaign) not like '%usa\_%') or  lower(campaign) like '%kas\_%' or ( lower(campaign) like '%192%' and lower(B.channel) like '%revx%' )
   then 'Saudi Arabia' else
     ...
     then 'UAE' else
       case when lower(campaign) like '%kw\_%' or lower(campaign) like '%kuw%' or ( lower(campaign) like '%117%' and lower(B.channel) like '%revx%' )
       then 'Kuwait' else
...
drop table if exists tajawal_bi.fact_affiliate_data_intermediate;
create table if not exists tajawal_bi.fact_affiliate_data_intermediate
row format delimited fields terminated by "\t"
as
select 
channel_group, channel, group_name, gadate, sourcemedium,
case when lower(coalesce(device_category, 'null')) = 'null' then 'NA' else device_category end as device_category,
campaign, 
...
view_id,
product,
case when ltrim(rtrim(lower(pos))) = 'uae' then 'United Arab Emirates' else pos end as pos,
case when lower(coalesce(mobile_os, 'NA')) = 'null' then 'NA' else coalesce(mobile_os, 'NA') end as mobile_os,
language,
brand_nonbrand
from (
...
group by
channel_group, channel, group_name, gadate, sourcemedium,
case when lower(coalesce(device_category, 'null')) = 'null' then 'NA' else device_category end,
campaign, 
view_id,
product,
case when ltrim(rtrim(lower(pos))) = 'uae' then 'United Arab Emirates' else pos end,
case when lower(coalesce(mobile_os, 'NA')) = 'null' then 'NA' else coalesce(mobile_os, 'NA') end,
language,
brand_nonbrand
;

drop table if exists tajawal_bi.fact_affiliate_data;
alter table tajawal_bi.fact_affiliate_data_intermediate rename to tajawal_bi.fact_affiliate_data;
